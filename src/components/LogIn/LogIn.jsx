...
export default function LogIn({ setStatusAuthentication }) {
  const url = `https://7051-37-115-217-3.ngrok-free.app/api/v1/public/auth`;
  const [showHideElement1, setShowHideElement1] = useState(false);
  const [statusBtn, setStatusBtn] = useState(true);
  const dispatch = useDispatch();
  const {
  ...
  const onSubmit = data => {
    axios
      ...
      .then(response => {
        const url = `https://7051-37-115-217-3.ngrok-free.app/api/v1/public/users/${response.data.userReference}`;
        axios.get(url).then(res => {
          dispatch(setUser(res.data));
        });
      })
